...
const schema = yup.object().shape({
  email: yup.string().required("Email is required").max(100, "Max length exceeded").min(6,"Minimum length 6 character"),
  name: yup.string().required("Name is required").max(100, "Max length exceeded").min(6,"Minimum length 6 character"),
  password: yup.string().required("Password is required").max(100, "Max length exceeded").min(6,"Minimum length 6 character"),
});
  ...
  <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
    <div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
      <form onSubmit={handleSubmit(onSubmit)}>
        <div>
          <label htmlFor="email" className="block text-sm font-medium leading-5  text-gray-700">Your Name</label>
          <div className="mt-1 relative rounded-md shadow-sm">
            <input {...register(("name"))} id="email" placeholder="Your Name" type="text" required className="appearance-none block w-full px-3 py-2 border border-gray-300 rounded-md placeholder-gray-400 focus:outline-none focus:shadow-outline-blue focus:border-blue-300 transition duration-150 ease-in-out sm:text-sm sm:leading-5" />
            {errors.name && (
        <span className="text-red-600" role="alert">{errors?.name?.message as ReactNode}</span>
      )}
          </div>
        </div>
        <div className="mt-6">
