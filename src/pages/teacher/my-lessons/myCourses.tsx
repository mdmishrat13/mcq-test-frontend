...
const MyCourses = () => {
  ...
  useEffect(() => {
    const getCourses = async () => {
      ...
      try {
        const course = await api.get("import.meta.env.VITE_BASE_URL_BACKEND/api/v1/courses")
        if (courses) {
    ...
    <div className='grid md:grid-cols-2 items-center center gap-2 py-4'>
      {courses?.map((course:any) => (
        <div key={course._id} className="w-full h-full p-6 bg-gray-200 border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
          ...
          <Link to={`import.meta.env.VITE_BASE_URL_FRONTEND/teacher/dashboard/create/${course._id}`} className="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg">
            Open Course
